...
# ====== قسم المدير (محسن لتقليل استدعاءات الشيت) ======
st.markdown("---")
...
if "pending_data" not in st.session_state:
    ...
    st.session_state["pending_data"] = pending_rows
...
# ====== عرض الشكاوى المعلقة من session_state ======
if st.session_state.get("admin_logged_in"):
    st.markdown("---")
    st.subheader("📋 الشكاوى في انتظار الاعتماد")
    pending_rows = st.session_state["pending_data"]
                ...
                if col1.button(f"✅ تم الاعتماد - {comp_id}", key=f"approve_{comp_id}"):
                    if not signer_text.strip() and not signer_image_str:
                        ...
                        if safe_append(target_sheet, row_to_return):
                            st.session_state["pending_data"] = [r for r in st.session_state["pending_data"] if r[0] != comp_id]
