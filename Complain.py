...
# ====== عرض الإجراءات المردودة مع تبويبات لكل نوع ======
st.header("✅ الإجراءات المردودة:")
responded_notes = responded_sheet.get_all_values()
if len(responded_notes) > 1:
    # تحديث قائمة الأنواع ديناميكيًا
    types_in_responded = list({row[1] for row in responded_notes[1:] if row[1].strip()})
    if not types_in_responded:
        types_in_responded = ["لا يوجد نوع"]
    tabs = st.tabs(types_in_responded)
    for tab, t_type in zip(tabs, types_in_responded):
        with tab:
            filtered_notes = [row for row in responded_notes[1:] if row[1] == t_type]
            if filtered_notes:
                for i, row in enumerate(filtered_notes, start=2):
                    render_complaint(responded_sheet, i, row, in_responded=True)
            else:
                st.info("لا توجد شكاوى لهذا النوع حالياً.")
else:
