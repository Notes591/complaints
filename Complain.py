...
# ================= Manager Approval / Electronic Signature =================
# شاشة اعتماد المدير (توقيع إلكتروني) بدون تغيير أي شيء من الكود الأساسي

import base64, io
from PIL import Image

# دوال مساعدة للوصول لكلمة مرور المدير
...
def manager_approval_ui():
    ...
    if entered_pw:
        if current_pw is None or entered_pw == current_pw:
            ...
            # عرض الطلبات المرسلة للاعتماد
            st.subheader("📋 الطلبات المرسلة للاعتماد")
            try:
                approval_sheet = sheets_dict.get("ForApproval")
                approvals = approval_sheet.get_all_values() if approval_sheet else []
            except Exception:
                approvals = []
            ...
            for i, row in enumerate(approvals[1:], start=2):
                ...
                with st.expander(f"🆔 {comp_id} — {comp_type} ({added})"):
                    ...
                    col1, col2 = st.columns(2)
                    with col1:
                        if st.button("✅ اعتماد", key=f"ok_{comp_id}_{i}"):
                            date_now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            if safe_append(sheets_dict.get("Approvals"), [comp_id, comp_type, notes, action, date_now, "Manager", signature_b64 or ""]):
                                if approval_sheet:
                    ...
                    with col2:
                        if st.button("❌ رفض", key=f"rej_{comp_id}_{i}"):
                            if approval_sheet and safe_append(archive_sheet, [comp_id, comp_type, notes, action, added, "مرفوض من المدير", outbound, inbound]):
                                safe_delete(approval_sheet, i)
...
# استدعاء واجهة اعتماد المدير
manager_approval_ui()
# ================= End of Manager Approval =================
