...
for title in sheet_titles:
    try:
        ...
        try:
            ...
            st.error(f"خطأ في إنشاء ورقة: {title} - {e2}")
            raise
...
def safe_append(sheet, row_data, retries=5, delay=1):
    for attempt in range(retries):
        try:
            ...
            time.sleep(delay)
    st.error("❌ فشل append_row بعد محاولات متعددة.")
    return False
...
def safe_update(sheet, cell_range, values, retries=5, delay=1):
    for attempt in range(retries):
        try:
            ...
            time.sleep(delay)
    st.error("❌ فشل update بعد محاولات متعددة.")
    return False
...
def safe_delete(sheet, row_index, retries=5, delay=1):
    for attempt in range(retries):
        try:
            ...
            time.sleep(delay)
    st.error("❌ فشل delete_rows بعد محاولات متعددة.")
    return False
    ...
    for row in order_number_data:
        if len(row) > 1 and str(row[1]) == str(order_id):
            ...
            if delegate.strip().lower() == "aramex":
                ...
                return f"🚚 مشحونة مع مندوب ({delegate})"
            return "⏳ تحت المتابعة"
    return "⏳ تحت المتابعة"
...
# ====== دالة عرض الشكوى (render_complaint) ======
def render_complaint(sheet, i, row, in_responded=False, in_archive=False):
    ...
    signature_image = row[8] if len(row) > 8 else ""   # التوقيع (Base64)

    order_status = get_order_status(comp_id)

    with st.expander(f"🆔 {comp_id} | 📌 {comp_type} | 📅 {date_added} {restored} | {order_status}"):

        # ===== عرض التوقيع الإلكتروني الحقيقي =====
        if signature_image and str(signature_image).strip() != "":
            try:
                img_bytes = base64.b64decode(signature_image)
                img = Image.open(BytesIO(img_bytes))
                st.image(img, width=150, caption="✔ تم الاعتماد - التوقيع الإلكتروني")
            except:
                st.warning("⚠ لا يمكن عرض صورة التوقيع")
        # =========================================
        ...
        with st.form(key=f"form_{comp_id}_{sheet.title}"):
            ...
            if rw_record:
                st.info(
                    ...
                    f"الفاتورة: {rw_record['الْفَاتُورَة'] if 'الْفَاتُورَة' in rw_record else rw_record['الفاتورة']}\n"
            ...
            # ===== أزرار التفاعل (حفظ، حذف، أرشفة، نقل، طلب اعتماد) =====
            ...
            # زر طلب اعتماد
            submitted_request_approval = col5.form_submit_button("🔵 طلب اعتماد")
            ...
            if submitted_archive:
                if safe_append(
                    archive_sheet,
                    [comp_id, new_type, new_notes, new_action, date_added, restored, new_outbound, new_inbound, signature_image]
                ):
                    if safe_delete(sheet, i):
                        st.success("♻️ الشكوى انتقلت للأرشيف")

            # ===== تنفيذ: نقل بين النشطة ↔ المردودة =====
            if submitted_move:
                if not in_responded:
                    if safe_append(
                        responded_sheet,
                        ...
                        if safe_delete(sheet, i):
                            st.success("➡️ تم نقل الشكوى إلى المردودة")
                else:
                    if safe_append(
                        complaints_sheet,
                        [comp_id, new_type, new_notes, new_action, date_added, restored, new_outbound, new_inbound, signature_image]
                        ...
                        if safe_delete(sheet, i):
                            st.success("⬅️ تم إرجاع الشكوى للنشطة")

            # ===== تنفيذ: طلب اعتماد =====
            if submitted_request_approval:
                try:
                    safe_update(sheet, f"D{i}", [["🔵 بانتظار اعتماد المدير"]])
                    st.success("🔵 تم إرسال طلب الاعتماد للمدير")
                except Exception:
                    st.error("❌ فشل في إرسال طلب الاعتماد")
    ...
    for sheet_obj, in_responded, in_archive in [
        ...
        for i, row in enumerate(data[1:], start=2) if data else []:
            if len(row) > 0 and str(row[0]) == search_id:
                st.success(
                    f"✅ الشكوى موجودة في "
                    f"{'المردودة' if in_responded else 'الأرشيف' if in_archive else 'النشطة'}"
                )
...
with st.form("add_complaint", clear_on_submit=True):
    ...
    if submitted:
        ...
        if comp_id.strip() and comp_type != "اختر نوع الشكوى...":
            ...
            active_ids = [str(c.get("ID", "")) for c in complaints]
            responded_ids = [str(r.get("ID", "")) for r in responded]
            archive_ids = [str(a.get("ID", "")) for a in archive]

            # موجودة بالفعل؟
            if comp_id in active_ids or comp_id in responded_ids:
                st.error("⚠️ الشكوى مسجلة بالفعل")

            # استرجاع من الأرشيف
            elif comp_id in archive_ids:
                try:
                    archived_rows = archive_sheet.get_all_values()[1:]
                except Exception:
                    archived_rows = []

                for idx, row in enumerate(archived_rows, start=2):
                    if str(row[0]) == comp_id:
                        ...
                        restored_signature = row[8] if len(row) > 8 else ""
            ...
            # شكوى جديدة
                ...
                if action.strip():
                    if safe_append(
                        responded_sheet,
                        [comp_id, comp_type, notes, action, date_now, "", outbound_awb, inbound_awb, ""]
                    ):
                        st.success("✅ تمت إضافة الشكوى (مردودة)")
                else:
                    if safe_append(
                        complaints_sheet,
                        [comp_id, comp_type, notes, "", date_now, "", outbound_awb, inbound_awb, ""]
                    ):
                        st.success("✅ تمت إضافة الشكوى (نشطة)")
        else:
            st.error("⚠️ يجب إدخال رقم شكوى واختيار نوع صحيح")
...
if len(responded_data) > 1:
    ...
    types_in_responses = list({row[1] for row in responded_data[1:]})

    for complaint_type in types_in_responses:

        with st.expander(f"📌 نوع: {complaint_type}"):

            type_rows = [
                (i, row)
                for i, row in enumerate(responded_data[1:], start=2)
            ...
            for i, row in type_rows:
                ...
                if delivered and rw_record:
                    return_ready.append((i, row))
                elif delivered and not rw_record:
                    followup_ready.append((i, row))
                else:
                    others.append((i, row))

            if followup_ready:
                with st.expander("📋 جاهزة للمتابعة 1"):
                    for i, row in followup_ready:
                        render_complaint(responded_sheet, i, row, in_responded=True)

            if return_ready:
                with st.expander("📋 جاهزة للمتابعة 2"):
                    for i, row in return_ready:
                        render_complaint(responded_sheet, i, row, in_responded=True)

            if others:
                with st.expander("📋 غير جاهزة للمتابعة"):
                    for i, row in others:
                        render_complaint(responded_sheet, i, row, in_responded=True)
...
# ====== عرض الأرشيف ======
st.header("📦 الأرشيف:")
archived_data = archive_sheet.get_all_values()

if len(archived_data) > 1:
    ...
    for i, row in enumerate(archived_data[1:show_count], start=2):
        render_complaint(archive_sheet, i, row, in_archive=True)

    if len(archived_data) - 1 > show_count:
...
with st.form("add_aramex", clear_on_submit=True):
    ...
    if submitted:
        if order_id.strip() and status.strip() and action.strip():
            ...
            st.error("⚠️ يجب إدخال رقم الطلب + الحالة + الإجراء")


# ====== عرض طلبات معلق أرامكس ======
...
if len(aramex_archived) > 1:
    ...
    for i, row in enumerate(aramex_archived[1:], start=2):
        ...
        with st.expander(f"📦 أرشيف طلب {order_id}"):
            ...
            if col1.button(f"⬅️ إرجاع {order_id} إلى المعلق"):
                if safe_append(aramex_sheet, [order_id, status, date_added, action]):
                    if safe_delete(aramex_archive, i):
                        st.success(f"♻️ تم إرجاع {order_id} للمعلق")

            if col2.button(f"🗑️ حذف {order_id} نهائياً"):
                if safe_delete(aramex_archive, i):
                    st.warning(f"🗑️ تم حذف {order_id}")

else:
    st.info("لا توجد بيانات في أرشيف أرامكس.")

# ====== نهاية الملف ======
st.caption("✔ جميع العمليات تتم مباشرة مع Google Sheets — النظام محدث بالتوقيع الإلكتروني وطلب الاعتماد.")
